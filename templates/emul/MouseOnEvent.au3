...
#CS
    ...
    Version:        2.1
    Requirements:   AutoIt v3.3.10.2+, Developed/Tested on Win 7, with standard 3-buttons mouse device.
    Uses:           WinAPISys.au3, APISysConstants.au3, WinAPIGdi.au3, WinAPIMisc.au3, WindowsConstants.au3, GUIConstantsEx.au3, Timers.au3
    ...
    Notes:          
					...
					* Do not mix _MouseSetOnEvent and _MouseSetOnEvent_RI, can cause unpredictable behaviour.
					...
					* When using Obfuscator/Au3Stripper, make sure to add event functions to ignore list (#Obfuscator_Ignore_Funcs/#Au3Stripper_Ignore_Funcs).
					
    ChangLog:
			v2.1 [12.08.2015]
			* Now if $iBlockDefProc is 0 (or when using _MouseSetOnEvent_RI), user function $sFuncName called by PostMessage to prevent issues with hook (or WM_* message) stopage.
...
Func __MouseSetOnEvent_MainHandler($nCode, $wParam, $lParam)
	...
	For $i = 1 To $a__MSOE_Events[0][0]
		If $a__MSOE_Events[$i][0] = $iEvent Or $iWScrollDirection <> 0 Then
			...
			If $iBlockDefProc_Ret = 0 Then
				Local $hFunc = _WinAPI_CreateString($i)
				_WinAPI_PostMessage($h__MSOE_RI_HWND, $i__MSOE_WM_CALL, $hFunc, $iEvent)
...
Func __MouseSetOnEvent_MainHandler_RI($hWnd, $iMsg, $wParam, $lParam)
	...
	For $i = 1 To $a__MSOE_Events_RI[0][0]
		If $a__MSOE_Events_RI[$i][0] = $iEvent Or $iWScrollDirection <> 0 Then
			...
			Local $hFunc = _WinAPI_CreateString($i & 'RI')
			_WinAPI_PostMessage($h__MSOE_RI_HWND, $i__MSOE_WM_CALL, $hFunc, $iEvent)
...
Func __MouseSetOnEvent_WM_CALL($hWnd, $iMsg, $wParam, $lParam)
	...
	Local $iIndex = _WinAPI_GetString($wParam)
	_WinAPI_FreeMemory($wParam)
	
	Local $iEvent = Number($lParam)
	Local $sFuncName, $vFuncParam
	
	If StringInStr($iIndex, 'RI', 2) Then
		$iIndex = Number(StringTrimRight($iIndex, 2))
		
		If $iIndex <= 0 Or $iIndex > $a__MSOE_Events_RI[0][0] Then
			Return $GUI_RUNDEFMSG
		EndIf
	...
	Else
		...
		If $iIndex <= 0 Or $iIndex > $a__MSOE_Events[0][0] Then
			Return $GUI_RUNDEFMSG
		EndIf
...
Func __MouseSetOnEvent_DoubleClickExpire($hWnd, $iMsg, $iIDTimer, $dwTime)
	...
	For $i = 0 To UBound($aEvntVars) - 1
		...
		If TimerDiff($aData[$i__MSOE_DblClkData_iTimer]) > $a__MSOE_DblClk_Data[0] Then
			_Timer_KillTimer(0, $aData[$i__MSOE_DblClkData_hTimer])
